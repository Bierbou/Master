...
angle_tilt = 3.15 ## be carefull projector needs reflected angle values
...
if fit_esf_v:
    ...
    for j in range(powersteps):
        ...
        if ccd:
            ...
            flatfield =  fit.getdata(flats_v[j])
        if paxscan:
        ...
        for i in np.arange(pic_number):
            ...
            if ccd:
                img_vert = fit.getdata(data_vcopy.pop(0))                
            if paxscan:
...
if fit_esf_h:
    ...
    for j in range(powersteps):
        if pilatus:
            ...
            flatfield[:,244] = flatfield[:,241] +(flatfield[:,245]-flatfield[:,241])      
        if ccd:
            #for FIT format of ccd 
            flatfield =  fit.getdata(flats_h[j])
        if paxscan:
        ...
        for i in np.arange(pic_number):
            ...
            if ccd:
                img_hor = fit.getdata(data_hcopy.pop(0))                
            if paxscan:
...
if fit_esf_d:
    ...
    for j in range(powersteps):
        ...
        if ccd:
            #for FIT format of ccd 
            flatfield =  fit.getdata(flats_d[j])
        if paxscan:
        ...
        for i in np.arange(pic_number):
            ...
            if ccd:
                img_diag = fit.getdata(data_dcopy.pop(0))                
            if paxscan:
...
if plots:
    if fit_esf_v:
        ...
        plt.title('Horizontal spot size 80kvp')
    ...
    if fit_esf_h:
        ...
        plt.title('Vertical spot size 80kvp')
        plt.ylabel('Spot size [micron]')
        plt.xlabel('Power [Watts]')
        plt.legend((( magnification_z0, magnification_z1, magnification_z2, 'Mean spot size')) ,loc = 0,title = 'MAGNIFICATION')
    if fit_esf_d:        
        ...
        plt.title('Diagonal spot size 80kvp')
        plt.ylabel('Spot size [micron]')
        plt.xlabel('Power [Watts]')
        plt.legend((( magnification_z0, magnification_z1, magnification_z2, 'Mean spot size')) ,loc = 0,title = 'MAGNIFICATION')
