# -*- coding: utf-8 -*-
"""
Created on Tue Mar 31 10:57:28 2015
...
with open('/users/Baier/analysis_files/detector_psf_80kvp.txt') as f:
    psfs = list(f)
    
""" Standard data folder"""
data = "/data/DPC/local_setups/microfocus/samples/"
""" Folder containing the data"""
data_folder = "linespread_acquisition_flat_measurement_paxscan_80kvp"
"""   Vertical edges """
...
"""   Horizontal edges """
flatfield_h = 78126 # picturenumber of first flat
...
"""   Diagonal edges """
flatfield_d = 78130# picturenumber of first flat
...
filepath_pic = '/users/Baier/analysis_pictures/80kvpspot_sizes/'
...
proj_angle = False   ##### If you run the script the first time do this option it's very profitable ####
...
angle_tilt = 3.1564 ## be carefull projector needs reflected angle values
...
if proj_angle:
    ...
    flatfield = e17.io.h5read(sam_v_folder+"/ct/paximage_ct_%06d.h5"%(78062))["raw_data"]
    img_vert = e17.io.h5read(sam_v_folder+"/ct/paximage_ct_%06d.h5"%(78064))["raw_data"]
...
#============================================================================================================
range1 = np.array(np.arange(5,(power_range1+ 2* stepsize1),stepsize1))
range2 = np.array(np.arange(power_range2+ 2* stepsize2,200+ stepsize2,stepsize2))
...
if plots:
    if fit_esf_v:
        ...
        plt.figure('Results HORIZONTAL spot-size for 80kvp' )
        plt.axis(h)
        plt.plot(watts, spot_width_horizontal, lw=2 )
        plt.plot(watts, mean_spot_size_horizontal, lw=2, ls = '--',color = 'k')
        #plt.title('Horizontal spot size 80kvp',fontsize = 10)
        plt.xticks( np.arange(0, x_max_h,20) )
        plt.xticks(fontsize  = 12)
        plt.yticks( np.arange(0, y_max_h,20) )
        ...
        plt.savefig(filepath_pic+'Results Horizontal spot-size for 80kvp.pdf', format = 'pdf',dpi = 300)
    if fit_esf_h:
        x_max_v = (1.1*np.max(watts))
        y_max_v = (1.05*np.max(spot_width_vertical))
        v = [ 0., x_max_v,  10., y_max_v]
        plt.figure('Results VERTICAL spot-size for 80kvp')
        plt.axis(v)
        plt.plot(watts, spot_width_vertical, lw=2)
        plt.plot(watts, mean_spot_size_vertical, lw=2, ls = '--', color = 'k')
        #plt.title('Vertical spot size 80kvp',fontsize = 10)
        plt.xticks( np.arange(0, x_max_v,20) )
        plt.xticks(fontsize  = 12)
        plt.yticks( np.arange(0, y_max_v,20) )
        plt.yticks(fontsize  = 12)
        plt.ylabel('Spot size [$\mu$m]',fontsize = 12)
        plt.xlabel('Power [W]',fontsize = 12)
        plt.legend((( magnification_z0, magnification_z1, magnification_z2, 'Mean spot size')) ,loc = 0,title = 'MAGNIFICATION',fontsize = 12)
        plt.savefig(filepath_pic+'Results Vertical spot-size for 80kvp.pdf', format = 'pdf',dpi = 300)
    if fit_esf_d:
        x_max_d = (1.1*np.max(watts))
        y_max_d = (1.05*np.max(spot_width_diagonal))
        d = [ 0., x_max_d,  10., y_max_d]        
        plt.figure('Results DIAGONAL spot-size for 80kvp')
        plt.axis(d)
        plt.plot(watts, spot_width_diagonal, lw=2)
        plt.plot(watts, mean_spot_size_diagonal, lw=2, ls = '--', color = 'k')
        #plt.title('Diagonal spot size 80kvp',fontsize = 10)
        plt.xticks( np.arange(0, x_max_d,20) )
        plt.xticks(fontsize  = 12)
        plt.yticks( np.arange(0, y_max_d,20) )
        plt.yticks(fontsize  = 12)
        plt.ylabel('Spot size [$\mu$m]',fontsize = 12)
        plt.xlabel('Power [W]',fontsize = 12)
        plt.legend((( magnification_z0, magnification_z1, magnification_z2, 'Mean spot size')) ,loc = 0,title = 'MAGNIFICATION',fontsize = 12)
        plt.savefig(filepath_pic+'Results Diagonal spot-size for 80kvp.pdf', format = 'pdf',dpi = 300)
