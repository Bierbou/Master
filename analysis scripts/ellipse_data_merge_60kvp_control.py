# -*- coding: utf-8 -*-
"""
Created on Wed Jul 22 14:28:43 2015
...
f = np.loadtxt('/users/Baier/analysis_files/linespread_acquisition_flat_measurement_control_60kvp.txt', delimiter = ', ')
f = np.swapaxes(f,0,1)    
...
filename = 'ellipse_xycoords_60kvp_control.txt'
filename1 = 'ellipse_xycoords_60kvp_control.npy'
...
#                        Plots
#============================================================================================================
plt.figure('60kvp control cartes coords',figsize = (9,7))
...
plt.title('spot size in x-y coords 60kvp control')
plt.xlim((-165,165))
plt.xticks( np.arange(np.round(1.0138838722759793*np.min(hor_phi_h2)), np.round(1.0138838722759793*np.max(hor_phi_h1))+1,20) ,fontsize  = 12)
plt.yticks( np.arange(np.round(1.0547966879383999*np.min(vert_phi_v2)),np.round(1.0547966879383999*np.max(vert_phi_v1)+1),10) ,fontsize  = 12)
plt.ylabel(' x-axis [$\mu$m]',fontsize = 12)
plt.xlabel('x-axis [$\mu$m]',fontsize = 12)
plt.legend( loc = 0,fontsize = 10,title = 'Spot-sizes')
plt.savefig(filepath_pic+'Results xy coords 60kvp_control.pdf', format = 'pdf',dpi = 300)
