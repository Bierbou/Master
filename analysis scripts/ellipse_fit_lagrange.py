# -*- coding: utf-8 -*-
"""
Created on Mon Mar 16 13:58:49 2015
...
Notes:    Detailed explanations can be found in:
...
def _find_max_eigvec(S):
    ...
    l : float (the positive eigen value)    
    ...
    u = np.array([ a12 * a23 - (a13  - 2*l) * (a22 + l),
                   a12 * (a13  - 2*l) - a23 * a11,
                   a11 * (a22 + l) - a12 * a12])
...
def fit_ellipse(X):
    ...
    S22_inv = inv(S22)
    S22_inv_S21 = np.dot(S22_inv, S12.T)
...
def get_parameters(x):
    ...
    :Note:
        ...
        Parameters are given as the parameter of the canonical:
