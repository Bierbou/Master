...
f = np.loadtxt('/users/Baier/analysis_files/linespread_acquisition_flat_measurement_paxscan_80kvp.txt', delimiter = ', ')
f = np.swapaxes(f,0,1)    
...
filename = 'ellipse_xycoords_80kvp_paxscan.txt'
filename1 = 'ellipse_xycoords_80kvp_paxscan.npy'
...
#                        Plots
#============================================================================================================
plt.figure('80kvp cartes coords',figsize = (8,7))
...
plt.title('spot size in x-y coords 80kvp ')
plt.xlim((-120,120))
plt.ylim((-70,70))
plt.xticks( np.arange(np.round(1.05*np.min(hor_phi_h2)), np.round(1.05*np.max(hor_phi_h1))+1,20) ,fontsize  = 12)
plt.yticks( np.arange(np.round(1.08*np.min(vert_phi_v2)),np.round(1.08*np.max(vert_phi_v1)+1),10) ,fontsize  = 12)
plt.ylabel(' y-axis [$\mu$m]',fontsize = 12)
plt.xlabel('x-axis [$\mu$m]',fontsize = 12)
plt.legend( loc = 0,fontsize = 10,title = 'Spot-sizes')
plt.savefig(filepath_pic+'Results xy coords 80kvp.pdf', format = 'pdf',dpi = 300)
