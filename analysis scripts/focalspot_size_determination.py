...
filepath_pic = '/users/Baier/analysis_pictures/60kvpspot_sizes/'
...
#        fwhm_vert[j,2] = fwhm_vert_pix[2]        
        spot_size_horizontal[j,0] = np.sqrt(np.abs(fwhm_vert_pix[0]**2 - fwhm_det_psf_hor**2))/(magnification_z0-1.)
        spot_size_horizontal[j,1] = np.sqrt(np.abs(fwhm_vert_pix[1]**2 - fwhm_det_psf_hor**2))/(magnification_z1-1.) 
        spot_size_horizontal[j,2] = np.sqrt(np.abs(fwhm_vert_pix[2]**2 - fwhm_det_psf_hor**2))/(magnification_z2-1.)
...
if fit_esf_h:
    ...
    for j in range(powersteps):
        ...
        for i in np.arange(pic_number):               
            ...
            img_hor = (img_hor/(flatfield_hor*1.))[350:600,200:450]
        ...
        spot_size_vertical[j,1] = np.sqrt(fwhm_hor_pix[1]**2 - fwhm_det_psf_vert**2)/(magnification_z1-1.)
        spot_size_vertical[j,2] = np.sqrt(fwhm_hor_pix[2]**2 - fwhm_det_psf_vert**2)/(magnification_z2-1.)
...
if fit_esf_d:
    ...
    for j in range(powersteps):
        ...
        spot_size_diagonal[j,1] = np.sqrt(fwhm_diag_pix[1]**2 - fwhm_det_psf_diag**2 )/(magnification_z1-1.)
        spot_size_diagonal[j,2] = np.sqrt(fwhm_diag_pix[2]**2 - fwhm_det_psf_diag**2 )/(magnification_z2-1.)
...
if plots:
    if fit_esf_v:
        ...
        plt.savefig(filepath_pic+'Results Horizontal spot-size for 60kvp.pdf', format = 'pdf',dpi = 300)
    if fit_esf_h:
        ...
        plt.xlabel('Power [W]',fontsize = 12)
        plt.legend((( magnification_z0, magnification_z1, magnification_z2, 'Mean spot size')) ,loc = 0,title = 'MAGNIFICATION')
        plt.savefig(filepath_pic+'Results Vertical spot-size for 60kvp.pdf', format = 'pdf',dpi = 300)
    if fit_esf_d:
        ...
        plt.xlabel('Power [W]',fontsize = 12)
        plt.legend((( magnification_z0, magnification_z1, magnification_z2, 'Mean spot size')) ,loc = 0,title = 'MAGNIFICATION')
        plt.savefig(filepath_pic+'Results Diagonal spot-size for 60kvp.pdf', format = 'pdf',dpi = 300)
