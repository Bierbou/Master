...
data_folder = "linespread_acquisition_flat_measurement_paxscan_60kvp"
"""   Vertical edges """
sam_v_folder = data_folder+"/vertical_edge" # data folder 
flatfield_v = 77353 # picturenumber of first flat of vertical direction
"""   Horizontal edges """
sam_h_folder = data_folder+"/horizontal_edge" # data folder of horizontal direction
flatfield_h = 77357 # picturenumber of first flat
"""   Diagonal edges """
sam_d_folder = data_folder+"/diagonal_edge" # data folder of diagonal direction
flatfield_d = 77361# picturenumber of first flat
...
angle_tilt = 3.05 ## be carefull projector needs reflected angle v###tilt in projection yields from drift in flatfield and the correction is not exactalues
pixel_subdivision = 0.01
setup_len = 195.6
##### DETector psf's Varian Paxscan#########
fwhm_det_psf_hor = 440.
fwhm_det_psf_vert = 379.
fwhm_det_psf_diag = (fwhm_det_psf_hor +fwhm_det_psf_hor )/2
...
if proj_angle:
    ...
    if paxscan: ### take pictures with higher power -> bigger spot -> easier to find the right angle. Choose 80  watts
        flatfield = e17.io.h5read(data+sam_v_folder+"/paxscan/ct/paximage_ct_%06d.h5"%(77533))["raw_data"]
        img_vert = e17.io.h5read(data+sam_v_folder+"/paxscan/ct/paximage_ct_%06d.h5"%(77535))["raw_data"]
...
if fit_esf_v:
    ...
    for j in range(powersteps):         
        if pilatus:
            #for cbf pilatus format    
            flatfield,meta =  e17.io.cbfread(data+sam_v_folder+"/pilatus/ct/specadm_1_ct_%i.cbf"%(flatfield_v +j*12))
            flatfield[:,242] = flatfield[:,241] +(flatfield[:,245]-flatfield[:,241])/3 
            flatfield[:,243] = flatfield[:,241] +2*(flatfield[:,245]-flatfield[:,241])/3
            flatfield[:,244] = flatfield[:,241] +(flatfield[:,245]-flatfield[:,241])      
        if ccd:
...
#        fwhm_vert[j,2] = fwhm_vert_pix[2]        
        #spot_size_horizontal[j,0] = (fwhm_vert_pix[0] )/(magnification_z0-1.) - (fwhm_det_psf_hor/magnification_z0)
        #spot_size_horizontal[j,1] = (fwhm_vert_pix[1] )/(magnification_z1-1.) - (fwhm_det_psf_hor/magnification_z1)
        #spot_size_horizontal[j,2] = (fwhm_vert_pix[2] )/(magnification_z2-1.) - (fwhm_det_psf_hor/magnification_z2)
        
        spot_size_horizontal[j,0] = np.sqrt(((fwhm_vert_pix[0])**2 - (fwhm_det_psf_hor/magnification_z0)**2 )*m0)    
        spot_size_horizontal[j,1] = np.sqrt(((fwhm_vert_pix[1])**2 - (fwhm_det_psf_hor/magnification_z1)**2 )*m1)
        spot_size_horizontal[j,2] = np.sqrt(((fwhm_vert_pix[2])**2 - (fwhm_det_psf_hor/magnification_z2)**2 )*m2)
...
#        spot_size_vertical[j,0] = (fwhm_hor_pix[0] )/(magnification_z0-1.) - (fwhm_det_psf_hor/magnification_z0)
#        spot_size_vertical[j,1] = (fwhm_hor_pix[1] )/(magnification_z1-1.) - (fwhm_det_psf_hor/magnification_z1)
#        spot_size_vertical[j,2] = (fwhm_hor_pix[2] )/(magnification_z2-1.) - (fwhm_det_psf_hor/magnification_z2)
        
        spot_size_vertical[j,0] = np.sqrt((fwhm_hor_pix[0]**2 - (fwhm_det_psf_vert/magnification_z0)**2 )*m0)    
        spot_size_vertical[j,1] = np.sqrt((fwhm_hor_pix[1]**2 - (fwhm_det_psf_vert/magnification_z1)**2 )*m1)
        spot_size_vertical[j,2] = np.sqrt((fwhm_hor_pix[2]**2 - (fwhm_det_psf_vert/magnification_z2)**2 )*m2)
...
#        spot_size_diagonal[j,0] = (fwhm_diag_pix[0] )/(magnification_z0)#-1.) - (fwhm_det_psf_hor/magnification_z0)
#        spot_size_diagonal[j,1] = (fwhm_diag_pix[1] )/(magnification_z1)#-1.) - (fwhm_det_psf_hor/magnification_z1)
#        spot_size_diagonal[j,2] = (fwhm_diag_pix[2] )/(magnification_z2)#-1.) - (fwhm_det_psf_hor/magnification_z2)
        
        spot_size_diagonal[j,0] = np.sqrt((fwhm_diag_pix[0]**2 - (fwhm_det_psf_diag/magnification_z0)**2 )*m0)    
        spot_size_diagonal[j,1] = np.sqrt((fwhm_diag_pix[1]**2 - (fwhm_det_psf_diag/magnification_z1)**2 )*m1)
        spot_size_diagonal[j,2] = np.sqrt((fwhm_diag_pix[2]**2 - (fwhm_det_psf_diag/magnification_z2)**2 )*m2)
