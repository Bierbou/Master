# -*- coding: utf-8 -*-
"""
Created on Fri Oct 16 13:07:24 2015
...
filename = 'ellipse_xycoords_40kvp_paxscan.txt'
filename1 = 'ellipse_xycoords_40kvp_paxscan.npy'
...
#                        Plots
#============================================================================================================
plt.figure('40kvp cartes coords',figsize = (7,7))
...
plt.title('spot size in x-y coords 40kvp')
plt.ylim((-35,35))
plt.xticks( np.arange(np.round(1.2823426289647113*np.min(hor_phi_h2)), np.round(1.2823426289647113*np.max(hor_phi_h1)+1),20) ,fontsize  = 12)
plt.yticks( np.arange(np.round(1.0881044580279706*np.min(vert_phi_v2)),np.round(1.0881044580279706*np.max(vert_phi_v1)+1),5) ,fontsize  = 12)
plt.ylabel(' x-axis [$\mu$m]',fontsize = 12)
plt.xlabel('x-axis [$\mu$m]',fontsize = 12)
plt.legend( loc = 0,fontsize = 10,title = 'Spot-sizes')
plt.savefig(filepath_pic+'Results xy coords 40kvp.pdf', format = 'pdf',dpi = 300)
